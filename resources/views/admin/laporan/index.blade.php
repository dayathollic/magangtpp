...
<div class="content-wrapper">
    ...
    <section class="content-header">
        <table class="table table-borderless">
            <thead>
                <tr>
                    <th colspan="6"><a href="{{ route('laporan.create') }}"><button type="button"
                        ...
                        <form action="{{ url('/admin/laporan') }}" method="GET">
                            <div class="shadow rounded-pill input-group input-group " style="width: 246px">
                                <input class="form-control  rounded-pill" type="text" name="keyword"
                                    placeholder="Cari ....." value="{{ $keyword }}" style="height: 48px">
                ...
                <table class="table-responsive table-bordered">
                    <thead>
                        <tr>
                            ...
                            <th scope="col">Instansi Pekerjaan</th>
                            <th scope="col">Rak Dokumen</th>
                            <th scope="col">Laporan Pendahuluan</th>
                            <th scope="col">Laporan Antara</th>
                            <th scope="col">Laporan Akhir</th>
                            <th scope="col">Policy Brief</th>
                            <th scope="col">Naskah Akademik</th>
                            <th scope="col">Rancangan Peraturan</th>
                            <th scope="col">Produk Lainnya</th>
                    ...
                    <tbody>
                        @forelse($laporans as $laporan)
                            ...
                            <tr>
                                ...
                                <td>
                                    @if($laporan->sampul)
                                        <img src="{{ asset('storage/' .$laporan->sampul) }}"
                                            alt="" style="width:60%">
                                        {{-- <a class="text-decoration:none text-dark" target="blank" 
                                            href="{{ asset('storage/' .$laporan->sampul) }}">ADA</a>
                                        --}}
                                    @else
                                        <p> Tidak Ada</p>
                                    @endif
                                ...
                                <td>{{ $laporan->tahun }}</td>
                                <td>{{ $laporan->instansi_pekerjaan }}</td>
                                <td>{{ $laporan->rak_dokumen }}</td>
                                <td>
                                    @if($laporan->laporan_pendahuluan)
                                        <a class="text-decoration:none text-dark" target="blank"
                                            href="{{ asset('storage/' .$laporan->laporan_pendahuluan) }}">ADA</a>
                                    @else
                                        <p> Tidak Ada</p>
                                    @endif
                                </td>
                                <td>
                                    @if($laporan->laporan_antara)
                                        <a class="text-decoration:none text-dark" target="blank"
                                            href="{{ asset('storage/' .$laporan->laporan_antara) }}">ADA</a>
                                    @else
                                        <p> Tidak Ada</p>
                                    @endif
                                </td>
                                <td>
                                    @if($laporan->laporan_akhir)
                                        <a class="text-decoration:none text-dark" target="blank"
                                            href="{{ asset('storage/' .$laporan->laporan_akhir) }}">ADA</a>
                                    @else
                                        <p> Tidak Ada</p>
                                    @endif
                                </td>
                                <td>
                                    @if($laporan->policy_brief)
                                        <a class="text-decoration:none text-dark" target="blank"
                                            href="{{ asset('storage/' .$laporan->policy_brief) }}">ADA</a>
                                    @else
                                        <p> Tidak Ada</p>
                                    @endif
                                </td>
                                <td>
                                    @if($laporan->naskah_akademik)
                                        <a class="text-decoration:none text-dark" target="blank"
                                            href="{{ asset('storage/' .$laporan->naskah_akademik) }}">ADA</a>
                                    @else
                                        <p> Tidak Ada</p>
                                    @endif
                                </td>
                                <td>
                                    @if($laporan->rancangan_peraturan)
                                        <a class="text-decoration:none text-dark" target="blank"
                                            href="{{ asset('storage/' .$laporan->rancangan_peraturan) }}">ADA</a>
                                    @else
                                        <p> Tidak Ada</p>
                                    @endif
                                </td>
                                <td>
                                    @if($laporan->produk_lainnya)
                                        <a class="text-decoration:none text-dark" target="blank"
                                            href="{{ asset('storage/' .$laporan->produk_lainnya) }}">ADA</a>
                                    @else
                                        <p> Tidak Ada</p>
                                    @endif
                                </td>
                                <td>
                                    <div class="d-flex">
                                        {{-- Trigger modal --}}
                                        <a href="{{ route('laporan.edit', $laporan->id) }}"><i
                                                class="text-black d-block"><svg width="33" height="33"
                                                    viewBox="0 0 33 33" fill="none" xmlns="http://www.w3.org/2000/svg">
                                                    <rect width="33" height="33" rx="5" fill="#2B2B78" />
                                                    <path
                                                        d="M18.8055 11.6997L20.3006 13.1941M19.7669 10.3832L15.7242 14.4258C15.5154 14.6344 15.3729 14.9002 15.3148 15.1896L14.9414 17.0588L16.8106 16.6847C17.1 16.6268 17.3655 16.4849 17.5744 16.276L21.6171 12.2333C21.7386 12.1118 21.835 11.9676 21.9007 11.8089C21.9664 11.6502 22.0003 11.4801 22.0003 11.3083C22.0003 11.1365 21.9664 10.9663 21.9007 10.8076C21.835 10.6489 21.7386 10.5047 21.6171 10.3832C21.4956 10.2617 21.3514 10.1653 21.1927 10.0996C21.0339 10.0338 20.8638 10 20.692 10C20.5202 10 20.3501 10.0338 20.1914 10.0996C20.0326 10.1653 19.8884 10.2617 19.7669 10.3832V10.3832Z"
                                                        stroke="white" stroke-width="2" stroke-linecap="round"
                                                        stroke-linejoin="round" />
                                                    <path
                                                        d="M20.5885 18.4701V20.5878C20.5885 20.9622 20.4398 21.3213 20.175 21.5861C19.9102 21.8508 19.5511 21.9996 19.1767 21.9996H11.4118C11.0374 21.9996 10.6783 21.8508 10.4135 21.5861C10.1487 21.3213 10 20.9622 10 20.5878V12.8229C10 12.4485 10.1487 12.0894 10.4135 11.8246C10.6783 11.5599 11.0374 11.4111 11.4118 11.4111H13.5295"
                                                        stroke="white" stroke-width="2" stroke-linecap="round"
                                                        stroke-linejoin="round" />
                                                </svg>
                                            ...
                                            <button type="button" class="border-0" data-bs-toggle="modal" data-bs-target="#deleteUserModal_{{$laporan->id}}">
                                                ...
                                                <form  method="POST"
                                                    ...
                                                    @csrf
                                                    @method('delete')
                                                        <div class="modal fade" id="deleteUserModal_{{$laporan->id}}" tabindex="-1" role="dialog"
